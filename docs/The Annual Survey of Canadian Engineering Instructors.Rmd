...
opts_chunk$set(echo = FALSE, dev="svg", fig.retina = NULL, dpi=150, out.width = "500px", fig.align = "center")
```
...
```{r Q1 Table 2, fig.cap="Instructor Academic Title"}
...
```{r Q1b Table 3, fig.cap="Other Instructor Roles"}
...
```{r Q2 Table 4, fig.cap="Teaching Experience"}
...
```{r Q3&4 Table 5, fig.cap="Teaching focus and division of work time"}
u.vs.g <- survey_data %>% 
...
```{r Q5 Figure 1, fig.cap="Figure 1"}
survey_data %>% 
  select(c(10)) %>% 
...
```{r Q6 Figure 2, fig.cap="Figure 2"}
survey_data %>%  
  select(c(11:15)) %>% 
...
```{r Q7 Figure 3, fig.cap="Figure 3"}
survey_data %>%  
  select(c(18:22)) %>% 
...
```{r Q8 Table 6, fig.cap="Common problems with student learning"}
q.8 <- survey_data %>% 
  select(c(25:37)) %>% 
  colSums(na.rm=TRUE) %>% 
  data.frame(names(.),.)
...
```{r Q10 Figure 4, fig.cap="Figure 4"}
survey_data %>% 
  select(c(39:41)) %>% 
...
```{r Q11 Figure 5, fig.cap="Figure 5"}
survey_data %>% 
  select(c(42:44)) %>% 
...
```{r Q12 Figure 6, fig.cap="Figure 6"}
survey_data %>% 
  select(c(45:47)) %>% 
